...
%define config_htmldir /opt/3ps/www/html/python
...
#define altinstall to perform 'make altinstall' instead of 'make install'
#altinstall will make very attempt to not overwrite existing python installation
#files. It will result in only a 'python-2.7' binary, no 'python' binary.
%define altinstall 0
...
find "$RPM_BUILD_ROOT" -type f -print0 |
      xargs -0 egrep -l "/usr/local/bin/python|$RPM_BUILD_ROOT%{__prefix}/bin/python" | while read file
do
   FIXFILE="$file"
...
########
%post
echo "/opt/3ps/lib" > /etc/ld.so.conf.d/opt-python2.7.conf
/sbin/ldconfig
...
%if %{altinstall}
%else
%{__prefix}/%{libdirname}/pkgconfig/python.pc
%{__prefix}/share/*
%endif
...
%exclude %{__prefix}/%{libdirname}/python%{libvers}/lib2to3/tests
%endif
